...
app.post("/api/message", (req, res) => {
  ...
  // 메시지 해제: 빈 값이면 모든 클라이언트에 CLEAR_MESSAGE 전송
  if (!classId && !messageText) {
    const message = JSON.stringify({ type: "CLEAR_MESSAGE" });
    sockets.forEach((ws) => ws.send(message));
    return res.json({ success: true });
  }

  // 관리자(admin)는 모든 반에 메시지 전송 가능, 경고 없음, 전체 broadcast
  if (req.session.class === "admin") {
    const message = JSON.stringify({
      type: "SEND_MESSAGE",
